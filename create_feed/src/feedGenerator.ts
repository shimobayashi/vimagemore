...
export function generateFeed(regionalDomainName:string, imageTag:AWS.DynamoDB.DocumentClient.AttributeMap, images:AWS.DynamoDB.DocumentClient.ItemList) {
    const feed = new Feed({
    ...
    images.forEach(image => {
        feed.addItem({
            ...
            link: image.Link,
            description: imageToDescription(regionalDomainName, image),
...
function imageToDescription(regionalDomainName:string, image:AWS.DynamoDB.DocumentClient.AttributeMap) {
    const tagHtml = `<div class="tags">${image.Tags.join(', ')}</div>`;
    const src = `https://${regionalDomainName}/${image.Path}`;
